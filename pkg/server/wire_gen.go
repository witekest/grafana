...
func Initialize(cfg *setting.Cfg, opts Options, apiOpts api.ServerOptions) (*Server, error) {
	...
	dependencyRegisterer, err := secret.RegisterDependencies(cfg, secretDBMigrator)
	if err != nil {
		return nil, err
	}
	apiregistryService := apiregistry.ProvideRegistryServiceSink(dashboardsAPIBuilder, snapshotsAPIBuilder, featureFlagAPIBuilder, dataSourceAPIBuilder, folderAPIBuilder, identityAccessManagementAPIBuilder, queryAPIBuilder, notificationsAPIBuilder, userStorageAPIBuilder, apiBuilder, dependencyRegisterer)
